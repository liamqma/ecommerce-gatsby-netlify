  ...
  return (
    <tr
      ...
      <td
        ...
        <div
          css={{
            ...
            position: 'relative',
            border:
              'var(--media-border-width) solid rgba(var(--color-foreground), var(--media-border-opacity))',
      ...
      <td
        ...
        {product.prices.length > 1 && <span css={{ fontSize: '1.2rem' }}> {price.nickname}</span>}
        <div
          css={{
            color: 'var(--color-1)',
            fontSize: '1.4rem',
          ...
          <div
            css={{
              ...
              ':before': {
                ...
                boxShadow:
                  'var(--inputs-shadow-horizontal-offset) var(--inputs-shadow-vertical-offset) var(--inputs-shadow-blur-radius) rgba(var(--color-base-text), var(--inputs-shadow-opacity))',
                zIndex: '-1',
              },
              ':after': {
                pointerEvents: 'none',
            ...
            <input
              ...
              value={item.qty}
              min="0"
              aria-label="Quantity for eleven vessels 2"
              data-index="1"
            />
            <button
              onClick={() => updateItem({ ...item, qty: item.qty + 1 })}
...
const CartPage = () => {
  ...
  const totalPrice = items.reduce(
    (total, item) => total + (findPrice(item.id).price?.unit_amount || 0) * item.qty,
    0
  );
  ...
  const checkout = async () => {
    ...
    setIsError(false);
    axios
      .post('/.netlify/functions/create-checkout-session', items)
      .then(({ data }) => {
        setLoading(false);
...
export const Head = () => <title>Your cart</title>;
